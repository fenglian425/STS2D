...
def crop_mask(masks, boxes, loose_factor):
    """
    ...
    if loose_factor is not None:
        pw, ph = x2 - x1 , y2 - y1
        x1 = x1 - pw * loose_factor
        y1 = y1 - ph * loose_factor
        x2 = x2 + pw * loose_factor
        y2 = y2 + ph * loose_factor
...
def process_mask_upsample(protos, masks_in, bboxes, shape, loose_factor=0.3):
    """
    ...
    masks = crop_mask(masks, bboxes, loose_factor)  # CHW
    return masks.gt_(0.5)


def process_mask(protos, masks_in, bboxes, shape, upsample=False, loose_factor=0.3):
    """
    ...
    masks = crop_mask(masks, downsampled_bboxes, loose_factor)  # CHW
    if upsample:
        masks = F.interpolate(masks[None], shape, mode='bilinear', align_corners=False)[0]  # CHW
    return masks.gt_(0.5)


def process_mask_native(protos, masks_in, bboxes, shape, loose_factor=0.3):
    """
    ...
    masks = F.interpolate(masks[None], shape, mode='bilinear', align_corners=False)[0]  # CHW
    masks = crop_mask(masks, bboxes, loose_factor=loose_factor)  # CHW
    return masks.gt_(0.5)
