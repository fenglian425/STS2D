...
class YOLO:
    ...
    def load_weights(self, path, exclude=()):
        import torch
        from ultralytics.yolo.utils.torch_utils import intersect_dicts
        ckpt = torch.load(path, map_location='cpu')
        pretrained_state_dict = ckpt['model'].state_dict()  # to FP32
        ...
        state_dict = intersect_dicts(intersect_state_dict, self.model.state_dict(), exclude=[])  # intersect
    ...
    def train(self, **kwargs):
        ...
        if not overrides.get('resume'):  # manually set model only if not resuming
            ...
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml, finetune=finetune)
